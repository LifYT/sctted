...
FREE_VERSION_URL = "https://t.me/sacredvisuals/1" # Ссылка для бесплатной версии
...
async def buy_skip(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(BuyFlow.waiting_for_plan) # Сначала меняем стейт
...
async def user_close_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(ADMIN_ID, f"🚫 **Юзер @{callback.from_user.username} закрыл тикет.**")
    await callback.answer()
...
async def key_check(message: types.Message, state: FSMContext):
    ...
    if key in keys_db:
        ...
        await state.set_state(TicketFlow.in_ticket)
        await message.answer(f"✅ **Ключ активирован!**\n📦 Тариф: {PLANS.get(data['plan'])}\n💬 Теперь вы можете написать админу:", reply_markup=user_close_ticket_kb(), parse_mode="Markdown")
